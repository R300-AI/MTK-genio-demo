...
# Demo 固定背壓 queue，不對外暴露
input_queue = asyncio.Queue(maxsize=QSIZE)
output_queue = asyncio.Queue(maxsize=QSIZE)

# Windows 預設可顯示；Linux 無頭環境用 DISPLAY/WAYLAND_DISPLAY 判斷
can_display = bool(
    (os.name == "nt") or os.environ.get("DISPLAY") or os.environ.get("WAYLAND_DISPLAY")
)
...
# 注意：asyncio.run 需要 coroutine，因此用最小的 async 包裝來 await gather
async def _runner():
